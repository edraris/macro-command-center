"""
Data caching layer — live FRED / World Bank calls, derived snapshots, map dataset.
Each data type has an independent cache TTL (see config.CACHE_TTL).
"""
...
from data.country_data import COUNTRY_METADATA, get_country_metadata
...
def _fallback_series(country_key: str, metric_key: str) -> pd.DataFrame:
    ...
    drift = drift_map.get(metric_key, 0.15)
    rows = []
...
@st.cache_data(ttl=CACHE_TTL["composite"], show_spinner=False)
def _cached_country_series(country_key: str, metric_key: str) -> tuple[pd.DataFrame, str, str]:
    """
    Returns (frame, fetched_at_iso, source_label).
    Tries live API; falls back to curated static estimates.
    """
...
@st.cache_data(ttl=CACHE_TTL["map"], show_spinner=False)
        ...
        rows.append(
            {
                ...
                "debt_to_gdp":  meta["indicators"]["debt_to_gdp"],
                "current_account": meta["indicators"]["current_account"],
                "population":    meta["indicators"]["population"],
                "gdp_per_capita": meta["indicators"]["gdp_per_capita"],
                "currency":      meta["indicators"]["currency"],
                "as_of":         series_as_of,
            }
        )
    _dev_log("map", "world_map", "miss")
    return pd.DataFrame(rows), datetime.now().isoformat(timespec="seconds")
...
def get_indicator_series(country_key: str, metric_key: str) -> pd.DataFrame:
    ...
    _dev_log("composite", key, "hit" if key in seen else "lookup")
    seen.add(key)
    try:
...
def get_country_snapshot(country_key: str) -> dict:
    """
    Returns a flat dict of current indicator values + year-on-year deltas + metadata.
    """
    base = get_country_metadata(country_key)["indicators"].copy()

    # Get latest values from live/fallback series
    for metric_key in ("gdp_value", "gdp_growth", "inflation", "unemployment", "interest_rate"):
        ...
        if not frame.empty:
            ...
            if len(frame) > 1:
                base[f"{metric_key}_delta"] = round(
                    float(frame.iloc[-1]["value"] - frame.iloc[-2]["value"]), 2
                )
            else:
